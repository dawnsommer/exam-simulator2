(function(){
  ...
  async function readLocal({flush=true,yieldBetween=false}={}){
    ...
    for(const rec of (cat.forms||[])){
      ...
      if(progress||suspended||score){const key=B.entityKey(rec.id,rec.bankHash);forms[key]={key,kind:'form',formId:String(rec.id),bankHash:String(rec.bankHash||''),progress:progress||null,suspended:suspended||null,threeDigitScore:score,modifiedAt:deriveModified(progress,suspended,rec),deviceId};}
  ...
  function makeFormBackup(entity,meta){return {type:R.config.FORM_BACKUP_TYPE,schemaVersion:R.config.SCHEMA_VERSION,appId:R.config.CLOUD.appId,versionId:meta.versionId,createdAt:meta.updatedAt,deviceId:meta.deviceId,build:R.config.BUILD,formId:entity.formId,bankHash:entity.bankHash,contentHash:meta.contentHash,payload:payloadOf(entity)};}
  ...
  function validateBackup(obj,entry){
    ...
    if(entry.kind==='form'){if(obj.type!==R.config.FORM_BACKUP_TYPE)throw new Error(`Cloud backup type is invalid for ${entry.formId}.`);if(String(obj.formId)!==String(entry.formId)||String(obj.bankHash)!==String(entry.bankHash))throw new Error(`Cloud backup identity mismatch for ${entry.formId}.`);}else if(obj.type!==R.config.QBANK_BACKUP_TYPE)throw new Error('Cloud Qbank backup type is invalid.');
    return obj;
  }
  ...
  async function restoreCheckpoint(){const cp=await R.meta.get('preBackupRecoverySnapshot',null);if(!cp)throw new Error('No pre-backup recovery checkpoint is available.');const b=bridge();window.__STEP_SYNC_APPLYING_REMOTE=true;try{for(const rec of(cp.catalog?.forms||[])){const key=B.entityKey(rec.id,rec.bankHash),e=cp.forms?.[key];if(e){if(e.progress)await b.writeFormProgressText(rec.id,JSON.stringify(e.progress),rec.bankHash);else await b.deleteFormProgress(rec.id);await b.writeFormSuspendedText(rec.id,e.suspended?JSON.stringify(e.suspended):null,rec.bankHash);await b.setThreeDigitScore(rec.id,e.threeDigitScore||'');}}if(cp.qbank?.progress)await b.writeQbankText(JSON.stringify(cp.qbank.progress));else await b.writeQbankText(null);await b.refresh();}finally{window.__STEP_SYNC_APPLYING_REMOTE=false;}return true;}

  async function applyBackup(entry,backup){validateBackup(backup,entry);const b=bridge(),payload=restoreFromCloud(backup.payload);window.__STEP_SYNC_APPLYING_REMOTE=true;try{if(entry.kind==='qbank'){await b.writeQbankText(JSON.stringify(payload?.progress||{}));}else{const cat=await b.catalog(),rec=(cat.forms||[]).find(x=>String(x.id)===String(entry.formId));if(!rec)throw new Error(`${entry.formId} is not loaded locally. Import the matching form first.`);if(String(rec.bankHash||'')!==String(entry.bankHash||''))throw new Error(`Form version mismatch for ${entry.formId}; cloud backup was not applied.`);if(payload?.progress)await b.writeFormProgressText(entry.formId,JSON.stringify(payload.progress),entry.bankHash);else await b.deleteFormProgress(entry.formId);await b.writeFormSuspendedText(entry.formId,payload?.suspended?JSON.stringify(payload.suspended):null,entry.bankHash);await b.setThreeDigitScore(entry.formId,payload?.threeDigitScore||'');}await b.refresh();}finally{window.__STEP_SYNC_APPLYING_REMOTE=false;}}
  async function applyDeletion(entry){const b=bridge();window.__STEP_SYNC_APPLYING_REMOTE=true;try{if(entry.kind==='qbank')await b.writeQbankText(null);else{const cat=await b.catalog(),rec=(cat.forms||[]).find(x=>String(x.id)===String(entry.formId));if(!rec)throw new Error(`${entry.formId} is not loaded locally. Import/restore the matching form library first.`);if(String(rec.bankHash||'')!==String(entry.bankHash||''))throw new Error(`Form version mismatch for ${entry.formId}; deletion tombstone was not applied.`);await b.deleteFormProgress(entry.formId);await b.writeFormSuspendedText(entry.formId,null,entry.bankHash);await b.setThreeDigitScore(entry.formId,'');}await b.refresh();}finally{window.__STEP_SYNC_APPLYING_REMOTE=false;}}
