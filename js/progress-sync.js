(function(){
  ...
  function ensureSurface(){
    installStyles();const tabBar=document.querySelector('.modern-sidebar');if(tabBar&&!document.getElementById('progressSyncTab')){const t=document.createElement('button');t.id='progressSyncTab';t.className='menu-tab';t.type='button';t.innerHTML='Progress Backup <span class="sync-tab-dot"></span>';const settings=document.getElementById('settingsTab');settings?tabBar.insertBefore(t,settings):tabBar.appendChild(t);}if(document.getElementById('progressSyncPanel')){renderUi();return;}
  ...
  async function boot(){
    ...
    ensureSurface();
    /* Bounded retries only. Do not observe the whole document: renderUi() itself mutates the DOM, and a missing tab target can otherwise create a self-triggering MutationObserver loop that exhausts Chromium/Safari. */
    [250,1000,2500].forEach(delay=>setTimeout(()=>{if(!document.getElementById('progressSyncTab')||!document.getElementById('progressSyncPanel'))ensureSurface();},delay));
    ...
    if(enabled&&A.getState().authorized){setStatus('Checking cloud…','Local progress is ready. Progress Sync V2 will check version lineage when the browser is idle, before any automatic backup.');const startCheck=()=>syncCheckpoint('App startup').catch(e=>console.warn('Startup progress check deferred/failed safely',e));if('requestIdleCallback'in window)requestIdleCallback(startCheck,{timeout:5000});else setTimeout(startCheck,2500);}else if(enabled)setStatus('Reconnect Google','Cloud backup is enabled, but this device no longer has a valid Worker session. Local progress is safe.');else setStatus('Disconnected','Progress is currently stored locally on this device.');
  }
