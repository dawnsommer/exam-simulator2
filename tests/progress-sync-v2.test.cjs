...
async function buildEnv({progress=null,score='',bankHash='BANK1'}={}){
  ...
  const state={catalog:{fileType:'StepExamSimulatorV75Catalog',forms:[{id:'N-11',bankHash,threeDigitScore:score,updatedAt:'2026-08-08T05:00:00.000Z'}]},progress:clone(progress),suspended:null,qbank:null};
...
async function localEntity(env){const l=await env.R.storage.localIndex({flush:false,yieldBetween:true});return l.index['N-11@@BANK1'];}
async function seedCloud(env,cloudProgress,version='v10'){
  ...
  env.drive.addJson(env.R.config.MANIFEST_FILE,{type:env.R.config.MANIFEST_TYPE,schemaVersion:2,appId:'exam-simulator2',updatedAt:'2026-08-08T05:00:00.000Z',forms:{[key]:{formId:'N-11',bankHash:'BANK1',currentVersionId:version,previousVersionId:null,driveFileId:pf.id,previousDriveFileId:null,checksum:e.contentHash,sizeBytes:+pf.size,updatedAt:'2026-08-08T05:00:00.000Z',deviceId:'DEV-CLOUD',deleted:false}},qbank:null});
...
T('L. Daily snapshots one/day and max 7',async()=>{const e=await buildEnv({progress:prog(0)}),seed=await seedCloud(e,prog(0));const old=[];for(let i=0;i<8;i++)old.push({id:'old'+i,type:'daily',createdAt:`2026-07-${String(i+1).padStart(2,'0')}T00:00:00.000Z`,day:`2026-07-${String(i+1).padStart(2,'0')}`,entities:{}});e.drive.addJson(e.R.config.HISTORY_MANIFEST_FILE,{type:e.R.config.HISTORY_MANIFEST_TYPE,schemaVersion:1,appId:'exam-simulator2',updatedAt:'2026-08-01T00:00:00.000Z',snapshots:old});e.state.progress=prog(1);await setMeta(e,seed.key,{baseCloudVersionId:'v10',dirty:true});await e.R.sync.backupNow({reason:'day first'});let h=history(e),daily=h.snapshots.filter(s=>s.type==='daily');assert.equal(daily.length,7);assert.equal(daily.filter(s=>s.day==='2026-08-08').length,1);const ce=currentEntry(e,seed.key);e.state.progress=prog(2);await setMeta(e,seed.key,{baseCloudVersionId:ce.currentVersionId,dirty:true});await e.R.sync.backupNow({reason:'same day second'});h=history(e);daily=h.snapshots.filter(s=>s.type==='daily');assert.equal(daily.length,7);assert.equal(daily.filter(s=>s.day==='2026-08-08').length,1);});
